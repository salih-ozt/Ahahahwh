    ...
    try {
        ...
        if (hasMedia) {
            ...
            if (isVideo) {
                ...
                // Ham dosyayı hem temp'e hem de uploads/videos'a kopyala
                // → rawServedPath anında yayınlanabilir (unoptimize ama oynatılabilir)
                await fs.copyFile(file.path, tempPath);
                await fs.copyFile(file.path, rawServedPath); // ⚡ hemen oynatılabilir kopya
                await fs.unlink(file.path).catch(() => {});

                // ⚡ Ham dosya zaten var → anında oynatılabilir URL
                media     = `/uploads/videos/${videoId}_raw.mp4`;
                mediaType = 'video';
